...
if audio_file is not None:
    if "transcribed_text" not in st.session_state or st.session_state.get("last_audio") != audio_file:
        try:
            ...
            st.warning(f"Speech-to-text not available: {str(e)}")
...
if "current_triage_result" in st.session_state and "current_patient" in st.session_state:
    ...
    with res_col1:
        st.markdown(
            f'<div class="esi-rec-panel esi-rec-{result.esi_level}">'
            f'<div class="esi-level-num">{result.esi_level}</div>'
            f'<div style="color:#5B6472;font-size:0.85rem;margin-top:4px;">LEVEL</div>'
            f'<div class="esi-confidence" style="margin-top:12px;">{conf_pct}%</div>'
            f'<div style="color:#5B6472;font-size:0.75rem;">confidence</div>'
            f'</div>',
    ...
    st.divider()
    st.subheader("Clinician Decision Review")
    ...
    with act_col2:
        with st.expander("Override Recommendation", expanded=False):
            ...
            if st.button("Submit Override", use_container_width=True):
                ...
                overridden_triage = TriageResult(
                    patient_id=pat.id,
                    esi_level=override_esi,
                    news2_score=result.news2_score,
                    confidence=result.confidence,
                    justification=f"Clinician Override ({override_reason}): {override_note}",
